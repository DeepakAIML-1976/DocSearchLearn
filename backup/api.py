...
def generate_summary(context, query):
    response = client.chat.completions.create(
        model="gpt-4o-mini",
        messages=[
            {"role": "system", "content": "You are an expert in Oil & Gas plant design. Summarize concisely."},
...
async def search(query: str = Query(...), top_k: int = 5):
    ...
    for i, idx in enumerate(indices[0]):
        ...
        # Apply feedback boost dynamically
        score -= 0.1 * feedback_scores.get(filename, 0)
        results.append({
            "rank": i + 1,
            "filename": filename,
    ...
    summary = generate_summary(combined_text, query)
...
async def download(filename: str):
    ...
    if os.path.exists(file_path):
        with open(DOWNLOAD_FILE, "a", newline="") as f:
            writer = csv.writer(f)
            writer.writerow([filename, datetime.now()])
        return FileResponse(file_path, filename=filename)
    return {"error": "File not found"}

@app.post("/feedback")
async def feedback(query: str, filename: str, feedback: str):
    with open(FEEDBACK_FILE, "a", newline="") as f:
        writer = csv.writer(f)
        writer.writerow([query, filename, feedback, datetime.now()])
    update_feedback_cache(filename, feedback)
